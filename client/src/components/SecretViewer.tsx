  ...
  if (isBurned) {
    return (
      <div className="h-full w-full flex flex-col items-center justify-center space-y-8 animate-in fade-in duration-700 bg-black">
        ...
        <a href="/" className="btn-action h-12 px-10">
          RETURN_TO_BASE
        </a>
      </div>
    );
    ...
    return (
      <div className="h-full w-full flex flex-col bg-black animate-in fade-in duration-700">
          ...
          <div className="flex-1 flex flex-col space-y-4 z-10 overflow-hidden corner-accent corner-accent-tl corner-accent-tr corner-accent-bl corner-accent-br p-2">
            <div className="shrink-0 flex justify-between items-center px-3">
              ...
              <button
                ...
                className="text-[8px] font-black uppercase tracking-[0.4em] text-white/40 hover:text-white transition-colors border-b border-white/10 pb-0.5"
              >
            ...
            <textarea
              ...
              cols={100}
              rows={20}
              className="flex-1 w-full bg-black/40 border border-white/5 p-8 font-mono text-[10px] text-white/70 outline-none resize-none no-scrollbar leading-relaxed"
            />
            ...
            <div className="shrink-0 p-5 bg-white/[0.01] border border-white/5">
              <p className="text-[8px] text-white/20 uppercase tracking-widest leading-relaxed">
                SESSION_VOLATILE_MEMORY. PURGE_RECOMMENDED_UPON_FINALIZE.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="shrink-0 grid grid-cols-2 gap-px bg-white/5 border-t border-white/5">
          <button onClick={handleBurn} className="bg-black h-16 text-[9px] font-black uppercase tracking-[0.5em] text-white/20 hover:text-white transition-all">
            EXECUTE_PURGE
          </button>
          <a href="/" className="bg-white h-16 flex items-center justify-center text-[9px] font-black uppercase tracking-[0.5em] text-black hover:bg-white/90 transition-all">
            FINALIZE_SESSION
          </a>
        </div>
      </div>
          ...
          <div className="w-full space-y-8 corner-accent corner-accent-tl corner-accent-tr corner-accent-bl corner-accent-br p-8 md:p-12 bg-white/[0.01] border border-white/5">
            <div className="space-y-4">
              ...
              <input
                ...
                className="w-full bg-black border border-white/5 p-6 text-sm font-mono text-white/70 outline-none focus:border-white/20 transition-all placeholder:text-white/5 text-center tracking-[0.5em]"
              />
            </div>

            {error && (
              <div className="py-2 text-white/40 text-[8px] font-black uppercase tracking-widest text-center border border-white/5">
                FAULT: {error}
              </div>
            )}
            ...
            <button
              ...
              disabled={!key || isDecrypting}
              className="w-full btn-action h-14 disabled:opacity-10"
            >
