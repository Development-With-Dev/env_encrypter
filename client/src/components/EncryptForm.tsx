...
export default function EncryptForm() {
  ...
  const handleFileRead = useCallback((file: File) => {
    if (file.size > 512000) { setError('SIZE_LIMIT_EXCEEDED'); return; }
    const reader = new FileReader();
    reader.onload = (e) => {
      const text = e.target?.result as string;
      if (text) { setContent(text); setUploadedFileName(file.name); setError(null); }
    };
  ...
  const handleEncrypt = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { accessToken } = await createSecret({
        encryptedData, iv, isPasswordProtected: false, maxViews, expiresIn,
      });
        ...
        <div className="space-y-8">
          <div className="space-y-3">
            ...
            <div className="flex flex-col gap-4">
              <input readOnly value={result.url} className="w-full bg-[#0a0a0b] border border-zinc-900 p-6 text-[10px] font-mono text-zinc-400 outline-none selection:bg-zinc-800" />
              <button onClick={() => { navigator.clipboard.writeText(result.url); setCopySuccess(true); setTimeout(() => setCopySuccess(false), 2000); }} className="w-full btn-action h-14">
  ...
  return (
    <div className="w-full h-full max-h-[75vh] flex flex-col animate-in fade-in duration-700 overflow-hidden">
      <form onSubmit={handleEncrypt} className="flex-1 flex flex-col space-y-6 overflow-hidden">
        <div onClick={() => fileInputRef.current?.click()} className="shrink-0 cursor-pointer bg-[#050506] p-8 border border-zinc-900 transition-all hover:border-zinc-700 group relative">
        ...
        <div className="flex-1 flex flex-col space-y-3 overflow-hidden">
          ...
          <textarea required value={content} onChange={(e) => setContent(e.target.value)} placeholder="PLAINTEXT_BUFFER_INGEST..." className="flex-1 w-full bg-[#050506] border border-zinc-900 p-8 font-mono text-[10px] text-zinc-400 outline-none resize-none no-scrollbar placeholder:text-zinc-900 focus:border-zinc-700 transition-colors" />
        </div>
        <div className="shrink-0 grid grid-cols-1 md:grid-cols-2 gap-px bg-white/5 border border-white/5">
          <div className="bg-black p-6 space-y-4">
            ...
            <div className="flex gap-2">
              {[1, 5, 10].map((v) => (
                <button key={v} type="button" onClick={() => setMaxViews(v)} className={`flex-1 py-3 text-[9px] font-black uppercase tracking-widest transition-all ${maxViews === v ? 'bg-white text-black' : 'bg-white/5 text-white/30 hover:bg-white/10'}`}>
          ...
          <div className="bg-black p-6 space-y-4">
            ...
            <div className="flex gap-2">
              {[{ label: '01H', val: 3600 }, { label: '24H', val: 86400 }, { label: '07D', val: 604800 }].map((t) => (
                <button key={t.val} type="button" onClick={() => setExpiresIn(t.val)} className={`flex-1 py-3 text-[9px] font-black uppercase tracking-widest transition-all ${expiresIn === t.val ? 'bg-white text-black' : 'bg-white/5 text-white/30 hover:bg-white/10'}`}>
                  {t.label}
                </button>
              ))}
            </div>
        ...
        <button type="submit" disabled={isEncrypting || !content.trim()} className="shrink-0 w-full btn-action h-14 disabled:opacity-10">
